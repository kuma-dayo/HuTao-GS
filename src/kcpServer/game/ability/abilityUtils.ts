...
export default class AbilityUtils {
  ...
  eval(ability: AppliedAbility, val: DynamicFloat | DynamicInt, defVal = 0): number {
  ...
  getCaster(): Entity {
    ...
    if ((<ClientGadget>entity).ownerEntityId != null)
      return entityManager.getEntity((<ClientGadget>entity).ownerEntityId)
    return entity
  }
    ...
    for (const nearbyEntity of nearbyEntities) {
      if (
        ...
        !this.isInsideShape(
          nearbyEntity,
          ShapeName,
          this.getShapeCenter(CenterBasedOn),
          this.eval(ability, SizeRatio) || 1
        )
      )
        continue
  ...
  getTargetList(
    ability: AppliedAbility,
    config: HealHP | LoseHP | ReviveAvatar | ConfigAbilityPredicate,
    target: Entity
  ): Entity[] {
    const { manager } = this
    const { entity } = manager

...
export default class World extends BaseClass {
  ...
  async destroy() {
    ...
    while (playerList.length > 0) {
      ...
      if (player.isHost()) await this.leave(player.context)
        ...
        await this.leave(
          player.context,
          QuitReasonEnum.KICK_BY_HOST_LOGOUT,
  ...
  async getScene(sceneId: number, fullInit = true) {
  ...
  async leave(
    ...
    quitReason: QuitReasonEnum = QuitReasonEnum.INVALID,
    clientReconnectReason: ClientReconnectReasonEnum = ClientReconnectReasonEnum.CLIENT_RECONNNECT_NONE
  ) {
    ...
    if (!isHost)
      await game.chatManager.sendPublic(
        this,
        0,
        ChatChannel.createSystemHint(player, SystemHintTypeEnum.CHAT_LEAVE_WORLD)
      )
  ...
  async changeToMp() {
    ...
    await currentScene.join(
      ...
      SceneEnterReasonEnum.HOST_FROM_SINGLE_TO_MP
    )
  }
  ...
  async changeToSingle() {
    ...
    await this.leave(
      host.context,
      QuitReasonEnum.HOST_NO_OTHER_PLAYER,
      ClientReconnectReasonEnum.CLIENT_RECONNNECT_QUIT_MP
    )
  }

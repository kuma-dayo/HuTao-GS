...
export default class OpenSSL {
  ...
  static async generateRsaPrivateKey(out: string, size = 2048) {
  ...
  static async generateCert(csr: string, ca: string, caKey: string, cnf: string, out: string) {
    await execCommand(
      `openssl x509 -req -days 825 -in "${csr}" -CA "${ca}" -CAkey "${caKey}" -CAcreateserial -out "${out}" -extensions req_ext -extfile "${cnf}"`
    )
  }
  ...
  static async pemToXmlRsaPrivate(inp: string): Promise<string> {
    const { Modulus, PublicExponent, Prime1, Prime2, Exponent1, Exponent2, Coefficient, PrivateExponent } =
  ...
  static async pemToXmlRsaPublic(inp: string): Promise<string> {
    const { Modulus, Exponent } = OpenSSL.parseText(await execCommand(`openssl rsa -pubin -in "${inp}" -text -noout`))

    if (Modulus == null || Exponent == null) throw new TError("message.openssl.error.invalidPublicKey")

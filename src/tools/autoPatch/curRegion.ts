...
async function decryptResponse(keyId: number, data: string): Promise<string> {
  ...
  try {
    ...
    return rsaDecrypt(clientKeyPair.private, Buffer.from(response.content, "base64")).toString("base64")
  } catch (err) {
...
function query(ip: string, overrideSeed?: string) {
  ...
  return new Promise<void>((resolve, reject) => {
    const path = `/query_cur_region?version=${dispatchRegion}Win${version}&lang=3&platform=3&binary=1&time=38&channel_id=1&sub_channel_id=0&account_type=1&dispatchSeed=${
      overrideSeed == null ? dispatchSeed : overrideSeed
    }${keyId}`
